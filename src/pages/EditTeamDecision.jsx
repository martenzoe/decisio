...
export default function EditTeamDecision() {
  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      try {
        ...
        if (!res.ok) throw new Error(json.error || t("editTeamDecision.loadError"));
        ...
        setCriteria(
          Array.isArray(json.criteria)
            ? json.criteria.map((c) => ({
                ...c,
                importance:
                  typeof c.importance === "number" ? c.importance : Number(c.importance) || 0,
              }))
            : []
        );
        ...
        if (user && Array.isArray(json.evaluations)) {
          const userEvals = json.evaluations.filter(
  ...
  function isDuplicate(arr, key, value, idx) {
    ...
    return arr.some(
      (item, i) =>
        i !== idx && item[key]?.trim().toLowerCase() === value.trim().toLowerCase()
    );
  }
  ...
  const handleOptionChange = (idx, val) => {
    ...
    setOptions((opts) =>
      opts.map((o, i) =>
        i === idx
          ? { ...o, name: v, _dup: isDuplicate(opts, "name", v, idx) }
          : { ...o, _dup: false }
      )
    );
  };
  ...
  const addOption = () =>
    setOptions((opts) => [
      ...opts,
      { name: "", id: `opt-new-${Date.now()}-${Math.random()}`, _dup: false },
    ]);
  ...
  const handleCriterionChange = (idx, key, val) => {
    ...
    setCriteria((crits) =>
      crits.map((c, i) =>
        i === idx
          ? { ...c, [key]: v, _dup: isDuplicate(crits, "name", v, idx) }
          : { ...c, _dup: false }
      )
    );
  };
  ...
  const addCriterion = () =>
    setCriteria((crits) => [
      ...crits,
      { name: "", importance: "", id: `crit-new-${Date.now()}-${Math.random()}`, _dup: false },
    ]);
  const removeCriterion = (idx) =>
    setCriteria((crits) => crits.filter((_, i) => i !== idx));
  ...
  async function handleDeadlineChange(newDeadline) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("editTeamDecision.saveFailed"));
      setSuccess(t("editTeamDecision.deadlineSaved"));
  ...
  async function handleModeChange(e) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("editTeamDecision.loadError"));
      setSuccess(t("editTeamDecision.modeChanged"));
  ...
  async function handleSaveAll() {
    ...
    try {
      ...
      await fetch(`/api/decision/${id}/evaluate`, {
        ...
        body: JSON.stringify({
          evaluations: evaluations
            ...
            .map(({ option_id, criterion_id, value }) => ({
              option_id,
              criterion_id,
              value: Number(value),
            })),
      ...
      setSuccess(t("editTeamDecision.saved"));
      setTimeout(() => setSuccess(null), 4000);
      setReload((r) => r + 1);
      return true;
    } catch (e) {
      setError(e.message || t("editTeamDecision.saveFailed"));
      return false;
    } finally {
      setLoading(false);
    }
  ...
  async function handleStartAI() {
    try {
      ...
      if (!saveOK) {
        setError(t("editTeamDecision.saveFailed"));
        return;
      }
      ...
      if (!res.ok) throw new Error(data.error || t("editTeamDecision.aiError"));
      setSuccess(t("editTeamDecision.aiSuccess"));
  ...
  if (userRole === "viewer")
    return (
      <main className="max-w-4xl mx-auto p-8 bg-white dark:bg-gray-900 rounded-lg shadow-lg">
        <h1 className="text-3xl mb-8 text-gray-900 dark:text-gray-100">
          {t("editTeamDecision.viewerTitle")}
        </h1>
        <section className="mb-10">
          <h2 className="text-xl mb-4 text-gray-700 dark:text-gray-300">
            {t("editTeamDecision.viewerOptions")}
          </h2>
        ...
        <section>
          <h2 className="text-xl mb-4 text-gray-700 dark:text-gray-300">
            {t("editTeamDecision.viewerCriteria")}
          </h2>
          <ul className="pl-6 list-disc text-gray-800 dark:text-gray-200">
            {criteria.map((o) => (
              <li key={o.id}>{o.name}</li>
    ...
    <main className="min-h-screen bg-transparent p-6 text-gray-900 dark:text-gray-100">
      <div className="max-w-4xl mx-auto">
        ...
        <section
          ...
          aria-label={t("editTeamDecision.settingsAria")}
        >
          {/* Modus */}
          {(userRole === "admin" || userRole === "owner") && (
            <div>
              ...
              >
                <option value="manual">{t("editTeamDecision.manual")}</option>
                <option value="ai">{t("editTeamDecision.aiAuto")}</option>
              </select>
              {aiEvaluated && (
                <p className="mt-2 text-indigo-600 dark:text-indigo-400">
                  {t("editTeamDecision.aiLocked")}
                </p>
              )}
            </div>
          )}
          ...
          <div className="space-y-6">
            ...
            <div>
              <label
                ...
                className="block mb-2 font-semibold text-gray-800 dark:text-gray-200"
              >
                {t("editTeamDecision.description")}
              </label>
              <textarea
                id={descId}
                rows={3}
          ...
          {(userRole === "admin" || userRole === "owner") && (
            <div className="mt-6 max-w-sm">
              <label
                ...
                className="block mb-2 font-semibold text-gray-800 dark:text-gray-200"
              >
                {t("editTeamDecision.deadlineLabel")}
              </label>
              <div className="relative">
                ...
                <button
                  ...
                  disabled={disable}
                  aria-label={t("editTeamDecision.pickDateAria")}
              ...
              <div className="mt-2 flex flex-wrap gap-4 items-center text-sm text-gray-600 dark:text-gray-400">
                ...
                >
                  {t("editTeamDecision.clearDeadline")}
                </button>
                <span>
                  {deadline
                    ? `${t("editTeamDecision.currentPrefix")} ${new Date(
                        deadline
                      ).toLocaleString()}`
                    : t("editTeamDecision.noDeadline")}
                </span>
                {disable && (
                  <span className="text-red-500 dark:text-red-400">
                    {t("editTeamDecision.deadlineExpired")}
                  </span>
                )}
              </div>
            </div>
          ...
          <section>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
              {t("editTeamDecision.optionsHeading")}{" "}
              <span className="text-sm text-gray-500">
                {t("editTeamDecision.optionsNoDuplicates")}
              </span>
            </h3>
            {options.map((option, i) => (
              <div key={option.id} className="flex items-center space-x-3 mb-3">
                <input
                  ...
                  type="text"
                  value={option.name}
                  placeholder={t("editTeamDecision.optionPlaceholder", { number: i + 1 })}
                ...
                {option._dup && (
                  <span
                    ...
                    aria-label={t("editTeamDecision.duplicateAria")}
                    role="alert"
                  >
                    ⚠
                  </span>
                ...
                <button
                  ...
                  disabled={disable}
                  aria-label={t("editTeamDecision.deleteOptionAria", { number: i + 1 })}
          ...
          <section className="mt-10">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">
              {t("editTeamDecision.criteriaHeading")}{" "}
              <span className="text-sm text-gray-500">
                {t("editTeamDecision.criteriaNoDuplicates")}
              </span>
            </h3>
            {criteria.map((criterion, i) => (
              <div key={criterion.id} className="flex items-center space-x-3 mb-3">
                <input
                  ...
                  placeholder={t("editTeamDecision.criterionPlaceholder", { number: i + 1 })}
                ...
                <button
                  ...
                  disabled={disable}
                  aria-label={t("editTeamDecision.deleteCriterionAria", { number: i + 1 })}
                  className="text-red-600 hover:text-red-700 disabled:opacity-50"
                >
                  ×
                </button>
            ...
            <button
              ...
              className="bg-indigo-600 text-white px-5 py-3 rounded hover:bg-indigo-700 disabled:opacity-50"
            >
              {t("editTeamDecision.addCriterion")}
            </button>
          </section>
        </section>
        ...
        {!aiEvaluated && (
          <section className="max-w-6xl mx-auto mt-12 flex justify-end">
            ...
            >
              {loading ? t("editTeamDecision.saving") : t("editTeamDecision.save")}
            </button>
          </section>
        )}
        ...
        {mode === "ai" &&
          ...
          (userRole === "admin" || userRole === "owner") && (
            <section className="max-w-6xl mx-auto mt-8 flex justify-end">
              ...
              >
                {aiLoading
                  ? t("editTeamDecision.aiRunning")
                  : t("editTeamDecision.startAi")}
              </button>
            </section>
          )}
